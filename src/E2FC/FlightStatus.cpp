...
class KalmanFilter{
  ...
  void set_barometer_ground_altitude() {
    float output = 0;
    for (int i=0; i < 100; i++) {
        output += (HAL::bmp390.readPressure() / 100.0);
    }
    output = output / 100;
  ...
  float get_barometer_altitude() {
    float output;
    output = HAL::bmp390.readAltitude(GROUND_PRESSURE_HPA);
    return output;
  }
  ...
  float get_barometer_temperature() {
    float output;
    output = HAL::bmp390.temperature;
    return output;
  }

  float get_barometer_pressure() {
    float output;
    output = HAL::bmp390.pressure / 100.0;
    return output;
  }

  float get_acceleration_x() {
    float output;
    HAL::lsm6dso32.getEvent(&accel, &gyro, &temp);
  ...
  float get_gps_altitude() {
    // default is in mm
    return HAL::neom9n.getAltitude() / 1000;
  }

  float get_latitude() { 
    return HAL::neom9n.getLatitude() / 1e7;
  }

  float get_longitude() { 
    return HAL::neom9n.getLongitude() / 1e7;
  }

  uint8_t get_gps_sats() { 
    return (uint8_t)(HAL::neom9n.getSIV());
  }

  // float get_baro2_temp() { 
  //   return HAL::ms5607.GetTemp();
  // }

  void read_baro2() { 
    HAL::ms5607.ReadProm();
    HAL::ms5607.Readout();
    baro_pressure_2 = HAL::ms5607.GetPres();
    baro_temp_2 = HAL::ms5607.GetTemp();
  }

  void update_accel_2() { 
    if (HAL::kx134.dataReady())
    {
      HAL::kx134.getAccelData(&kxData);
    }
  ...
  void init(){
    ...
    if (HAL::neom9n.begin() == false) //Connect to the u-blox module using Wire port
    {
    ...
    if (!HAL::bmp390.begin_I2C(0x77)) {
      Serial.println("Could not find a valid BMP3 sensor, check wiring!");
      while (1);
    }

    if (!HAL::lsm6dso32.begin_I2C()) {
      while (1) {
        delay(10);
      }
    }

    HAL::lsm6dso32.setAccelRange(LSM6DSO32_ACCEL_RANGE_16_G);
    HAL::lsm6dso32.setGyroRange(LSM6DS_GYRO_RANGE_1000_DPS );
    HAL::lsm6dso32.setAccelDataRate(LSM6DS_RATE_3_33K_HZ);
    HAL::lsm6dso32.setGyroDataRate(LSM6DS_RATE_3_33K_HZ);


    HAL::bmp390.setIIRFilterCoeff(BMP3_IIR_FILTER_COEFF_3);
    HAL::bmp390.setOutputDataRate(BMP3_ODR_200_HZ);
    HAL::bmp390.performReading();
    ...
    // if (HAL::kx134.softwareReset())
    //   Serial.println("Reset.");

    // Give some time for the accelerometer to reset.
    // It needs two, but give it five for good measure.
    delay(2);
    HAL::kx134.enableAccel(false);
    HAL::kx134.setRange(SFE_KX134_RANGE64G);         // 16g for the KX134
    HAL::kx134.enableDataEngine(); // Enables the bit that indicates data is ready.
    // HAL::kx134.setOutputDataRate(); // Default is 50Hz
    HAL::kx134.enableAccel();

    if(HAL::ms5607.connect()>0) {
  ...
  // Packet definitions
  Comms::Packet imuPacket = {.id = 2};
  Comms::Packet baroPacket = {.id = 3};
  Comms::Packet gpsPacket = {.id = 4};
  Comms::Packet filteredPacket = {.id = 9};
  Comms::Packet eventsPacket = {.id = 12};
